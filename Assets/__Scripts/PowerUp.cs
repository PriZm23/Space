...
public class PowerUp : MonoBehaviour
{
    ...
    // Необычное, но удобное применение Vector2.x хранит
    // минимальное значение, а у - максимальное значение 
    ...
    public WeaponType type; // Тиа бонуса
    ...
    private void Awake()
    {
        ...
        // Quaternion.identity равносильно отсутствию поворота
    ...
    private void Update()
    {
        ...
        // а затем растворяется в течение 4 сек
        float u = (Time.time - (birthTime + lifeTime)) / fadeTime;
        // В течение lifeTime секунд значение u будет <= 0
        // затем оно станет положительным и через fadeTime
        // секунд станет больше 1
        ...
        {
            // Если бонус за границей экрана - уничтожить
            Destroy(gameObject);
        }
    }
    ...
    {
        // Эта функция вызывается классом Hero, когда игрок берёт бонус
        // Можно было бы реализовать эффект поглощения бонуса, уменьшая
        // его размеры в течение нескольких кадров, но пока просто уничтожим
        // this.gameObject
        Destroy(this.gameObject);
    }
}
